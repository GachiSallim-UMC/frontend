...
/**
 * 참여 중인 그룹 목록. 모바일은 세로 나열, 데스크톱은 2열 그리드입니다.
 * 그룹이 하나도 없을 때의 화면은 GroupEmptyState가 담당합니다.
 */
export const GroupList = () => {
  ...
  return (
    <div className="flex flex-col">
      <h2 className="mb-2 hidden text-left text-sm font-bold text-gray-800 lg:block">
        참여 중인 그룹
      </h2>
      <div className="flex flex-col gap-3 lg:grid lg:grid-cols-2">
        {groups.map(group => (
          <GroupCard key={group.id} group={group} onEnter={handleEnterGroup} />
        ))}
      </div>
    </div>
  );
