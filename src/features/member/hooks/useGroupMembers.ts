...
export const GROUP_MEMBER_QUERY_KEYS = {
  ...
  userList: (groupId: string | null, userId: string | null) =>
    [...GROUP_MEMBER_QUERY_KEYS.list(groupId), userId] as const,
};
...
export const useGroupMembers = (groupId: string | null) => {
  ...
  const query = useQuery({
    queryKey: GROUP_MEMBER_QUERY_KEYS.userList(groupId, userId),
    queryFn: () => memberApi.getGroupMembers(groupId as string),
    enabled: Boolean(accessToken && userId && groupId),
  });
