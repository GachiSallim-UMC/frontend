import { CustomButton, CheckboxModal } from '@/features/expense';
...
export function ExpenseDetailCard({ expense, onRefresh }: ExpenseDetailCardProps) {
  const {
    isModalOpen,
    setIsModalOpen,
    handleBulkSettle,
  ...
  const getSplitTypeText = (type: Expense['splitType']) => {
    switch (type) {
      case 'EQUAL': return '균등 분할 (n/n)';
      case 'CUSTOM': return '직접 입력';
      case 'RATIO': return '비율 분할 (%)';
      default: return '균등 분할 (n/n)';
    }
  };
    ...
    <div className='w-full bg-white p-[24px] rounded-[18px] flex flex-col gap-6 relative'>
      ...
      <div className='flex flex-col gap-4'>
        ...
        <div className='flex flex-col gap-3 text-button text-black'>
          {shares.length === 0 ? (
            <div className='text-caption text-gray-400 p-2'>분담 내역이 없습니다.</div>
          ) : (
            shares.map((share) => {
