...
export const choreApi = {
  ...
  getList: async (filter?: ChoreFilter): Promise<Chore[]> => {
    const { data } = await apiClient.get<ApiResponse<Chore[]>>(BASE, { params: filter });
  ...
  getDetail: async (id: string): Promise<Chore> => {
    const { data } = await apiClient.get<ApiResponse<Chore>>(`${BASE}/${id}`);
    return data.data;
  },

  /** 집안일 등록 */
  create: async (dto: CreateChoreDto): Promise<Chore> => {
    const { data } = await apiClient.post<ApiResponse<Chore>>(BASE, dto);
    return data.data;
  },

  /** 집안일 수정 */
  update: async (id: string, dto: UpdateChoreDto): Promise<Chore> => {
    const { data } = await apiClient.patch<ApiResponse<Chore>>(`${BASE}/${id}`, dto);
    return data.data;
  },
